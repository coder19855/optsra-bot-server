...
export default fp(async (fastify: FastifyInstance) => {
  ...
  const mapStrategiesWithVix = (
    ...
    // Style-aware bias bands. Scalper gets more aggressive (directional) strategy suggestions
    // even on moderately strong scores because the input score is already intraday-tilted.
    const sigThresh = fastify.utilsPlugin.getSignalThreshold(style);
    const moderate = Math.max(sigThresh + 8, 32);
    const strong = 68; // keep strong quite high (structural conviction)
    ...
    // NEUTRAL
    else if (score > -moderate && score < moderate) {
    ...
    // MODERATE BEARISH
    else if (score <= -moderate && score > -strong) {
