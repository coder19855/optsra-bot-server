...
export default fp(
  async (fastify: FastifyInstance) => {
    ...
    const calcTrendConfirmationScore = (
      ...
      // Filter out noise strikes
      const filtered = chain.filter((r) => (r.oi ?? 0) > 5000);

      // Net OI change across meaningful strikes
      const netOIChange = filtered.reduce((sum, r) => sum + (r.oich ?? 0), 0);

      // Price thresholds for NIFTY
      const priceUp = spotChangePercent > 0.02;
      const priceDown = spotChangePercent < -0.02;

      // OI thresholds for NIFTY
      const oiUp = netOIChange > 5000;
      const oiDown = netOIChange < -5000;
